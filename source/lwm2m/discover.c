                                    ...
                                    size_t bufferLen )
{
    ...
    for ( watcherP = observedP->watcherList; watcherP != NULL; watcherP = watcherP->next )
    {
        ...
        {
            /* multiple servers */
            nbiot_memmove( buffer + head, buffer, uriLen );
                                  ...
                                  size_t bufferLen )
{
    ...
    switch ( tlvP->type )
    {
        ...
        nbiot_memmove( buffer + head, LINK_ITEM_START, LINK_ITEM_START_SIZE );
        head = LINK_ITEM_START_SIZE;

        if ( parentUriLen > 0 )
        {
            if ( bufferLen - head < parentUriLen ) return -1;
            nbiot_memmove( buffer + head, parentUriStr, parentUriLen );
            head += parentUriLen;
        }

        if ( bufferLen - head < LINK_URI_SEPARATOR_SIZE ) return -1;
        nbiot_memmove( buffer + head, LINK_URI_SEPARATOR, LINK_URI_SEPARATOR_SIZE );
        ...
        if ( tlvP->type == LWM2M_TYPE_MULTIPLE_RESOURCE )
        {
            ...
            nbiot_memmove( buffer + head, LINK_ITEM_DIM_START, LINK_ITEM_DIM_START_SIZE );
            ...
            nbiot_memmove( buffer + head, LINK_ITEM_ATTR_END, LINK_ITEM_ATTR_END_SIZE );
            head += LINK_ITEM_ATTR_END_SIZE;
        }
        else
        {
            if ( bufferLen - head < LINK_ITEM_END_SIZE ) return -1;
            nbiot_memmove( buffer + head, LINK_ITEM_END, LINK_ITEM_END_SIZE );
            head += LINK_ITEM_END_SIZE;
        }

        nbiot_memmove( &uri, parentUriP, sizeof(lwm2m_uri_t) );
        ...
        case LWM2M_TYPE_OBJECT_INSTANCE:
        {
            ...
            if ( parentUriLen > 0 )
            {
                ...
                nbiot_memmove( uriStr, parentUriStr, parentUriLen );
                uriLen = parentUriLen;
            }
            else
            {
            ...
            nbiot_memmove( uriStr + uriLen, LINK_URI_SEPARATOR, LINK_URI_SEPARATOR_SIZE );
                        ...
                        uint8_t ** bufferP )
{
    ...
    head = 0;
    nbiot_memzero( &tempUri, sizeof(lwm2m_uri_t) );
    ...
    if ( head > 0 )
    {
        ...
        *bufferP = (uint8_t *)nbiot_malloc( head );
        if ( *bufferP == NULL ) return 0;
        nbiot_memmove( *bufferP, bufferLink, head );
    }
